...
```bash
OutDir=analysis/genome_alignment/circos/vs_AS22/cc03
mkdir -p $OutDir
```
#Transform our .fa files with the genomes in .txt files (both the sample and the reference genomes.)

```bash
QueryGenome=$(ls repeat_masked/*/*/filtered_contigs/*_contigs_hardmasked.fa| grep 'cc03')
ProgDir=~/git_repos/tools/seq_tools/circos
python $ProgDir/fasta2circos.py --genome $QueryGenome --contig_prefix "cc03_" > $OutDir/query_genome.txt
...
```bash
OutDir=analysis/genome_alignment/circos/vs_AS22/cc03
Coords=$(ls analysis/genome_alignment/mummer/c.albicans/cc03/cc03_vs_A22/cc03_vs_A22_coords.tsv)
ProgDir=~/git_repos/scripts/alternaria/pathogen/genome_alignment
$ProgDir/nucmer_coords2circos.py --inp_coords $Coords --queery_id cc03 --ref_id A22 > $OutDir/query_vs_ref_links.txt
...
echo "Order of unseen cc03 contigs and remaining cc03 contigs"
cat $OutDir/query_ref_genome.txt | grep -w -v -f tmp.txt | cut -f3 -d ' '| tr -d '\n' | sed 's/cc03/, cc03/g' | sed 's/A22/, A22/g'
```

```bash
, cc03_contig_12, A22_Ca22chrRA_C_albicans_SC5314, A22_Ca22chrM_C_albicans_SC5314, A22_Ca22chr7A_C_albicans_SC5314, A22_Ca22chr6A_C_albicans_SC5314, A22_Ca22chr5A_C_albicans_SC5314, A22_Ca22chr4A_C_albicans_SC5314, A22_Ca22chr3A_C_albicans_SC5314, A22_Ca22chr2A_C_albicans_SC5314, A22_Ca22chr1A_C_albicans_SC53145
```
```
OutDir=/home/vegasa/circos/vs_AS22/cc03
ProgDir=/home/vegasa/git_repos/scripts/circos_plots/genome_alignment/C.albicans/TLO_project/vs_A22/cc03
circos -conf $ProgDir/A22_vs_cc03_circos.conf -outputdir $OutDir
mv $OutDir/circos.png $OutDir/A22_vs_cc03_circos.png
mv $OutDir/circos.svg $OutDir/A22_vs_cc03_circos.svg
ls $PWD/$OutDir/A22_vs_cc03_circos.png
```
